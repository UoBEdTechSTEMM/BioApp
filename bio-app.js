...
function Run() {
  ...
  if (jQuery.isNumeric(document.getElementById("vmax").value)==false){vmax = 10;}
  else {vmax = parseFloat(document.getElementById("vmax").value);}
  if (jQuery.isNumeric(document.getElementById("km").value)==false){km = 7;}
  else {km = parseFloat(document.getElementById("km").value);}
  vmax = vmax * E;

  if (jQuery.isNumeric(document.getElementById("kic").value)==false){kic = 1;}
  else {kic = parseFloat(document.getElementById("kic").value);}

  if (jQuery.isNumeric(document.getElementById("kiu").value)==false){kiu = 1;}
  else {kiu = parseFloat(document.getElementById("kiu").value);}
  ...
  if ((jQuery.isNumeric(document.getElementById("S").value)) == false || (jQuery.isNumeric(document.getElementById("E").value)) == false || (jQuery.isNumeric(document.getElementById("off").value)) == false) {
    ...
    alert("The substrate is not soluble at this concentration")
  } else {
    if ((jQuery.isNumeric(document.getElementById("I").value)) == false && document.getElementById("inhib").value != 'none') {
      ...
      if (document.getElementById("inhib").value == "competitive") {
        v = base + off + Math.round(((vmax * S) / (km * (1 + I / kic) + S) + err) * 100) / 100;
      } else if (document.getElementById("inhib").value == "uncompetitive") {
        v = base + off + Math.round(((vmax * S) / (km + S * (1 + I / kiu)) + err) * 100) / 100;
      } else if (document.getElementById("inhib").value == "mixed") {
        v = base + off + Math.round(((vmax * S) / (km * (1 + I / kic) + S * (1 + I / kiu)) + err) * 100) / 100;
      } else {
        v = base + off + Math.round(((vmax * S) / (km + S) + err) * 100) / 100
      };
      ...
      costcalc();
      total = total + cost;
      var totaldis = total.toFixed(2);
      document.getElementById("cost total").innerHTML = "\u00A3" + totaldis;
      ...
      for (j = 0; j < arr[0].length; j++) {
        ...
        if (inhibitor) {
          csvString += arr[0][j] + "," + arr[1][j] + "," + arr[2][j] + "\n";
        } else {
          csvString += arr[0][j] + "," + arr[2][j] + "\n";
        };
      };
...
function costcalc() {
  ...
  if (jQuery.isNumeric(document.getElementById("E").value) == false) {
    e = 0
  } else {
    e = parseFloat(document.getElementById("E").value);
  };
  if (jQuery.isNumeric(document.getElementById("S").value) == false) {
    s = 0
  } else {
    s = parseFloat(document.getElementById("S").value);
  };
  if (jQuery.isNumeric(document.getElementById("I").value) == false) {
    i = 0
  } else {
  ...
  cost = (e * 2 + s * 3 + i * inh) / 10;
	var costdis = cost.toFixed(2);
  document.getElementById("cost per").innerHTML = "\u00A3" + costdis;
